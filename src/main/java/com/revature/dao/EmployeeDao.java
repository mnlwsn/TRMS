...
public class EmployeeDao {
	...
	public void insertEmployee(Employee em) {
		...
		try (Connection conn = ConnectionUtil.getConnection();) {
			...
			String sql = "INSERT INTO personnel (fname, lname, usrnm, pswrd, email, phone, dept, reportsto, amt_avail)"
						+ "VALUES(?,?,?,?,?,?,?,?,1000)";
			...
			ps.setLong(8, em.getSuperid());
			int affected = ps.executeUpdate();
			
			
			log.info(affected + " new employee successfully stored.");
		} catch (Exception e2) {
			log.info("New employee insert FAIL.");
			 e2.printStackTrace();
		} finally {
			close(ps);
		}
